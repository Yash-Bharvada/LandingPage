...
const FeatureSection = () => {
  ...
  return (
    <section id="features" className="py-20 px-6 bg-gradient-to-b from-gray-950 to-black text-white">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-16">
          <h2 className="text-4xl font-bold mb-4">Powerful Features</h2>
          <p className="text-xl text-gray-400 max-w-2xl mx-auto">Our AI-powered platform simplifies contract analysis and management</p>
        </div>
        ...
        >
          {features.map((feature, i) => (
            <motion.div 
              key={i} 
              className="bg-gray-900/80 backdrop-blur p-8 rounded-2xl border border-gray-800 hover:border-primary-500/50 shadow-xl hover:shadow-primary-500/10 transition-all duration-300 w-full max-w-sm"
              variants={itemVariants}
            >
              <div className="bg-gray-800/50 p-3 rounded-xl w-fit mb-6 mx-auto">
                {feature.icon}
              </div>
              <h3 className="text-2xl font-bold mb-3 text-center">{feature.title}</h3>
              <p className="text-gray-400 leading-relaxed text-center">{feature.desc}</p>
            </motion.div>
          ))}
        </motion.div>
      </div>
